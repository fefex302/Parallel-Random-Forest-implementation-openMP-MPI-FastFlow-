...
int DecisionTree::flatten_tree(TrainNode* node) {
    ...
    // 3. Ricorsione per i figli
    // Nota: dobbiamo usare una variabile temporanea per l'indice sinistro/destro
    // perché 'flat_nodes' potrebbe essere riallocato durante la chiamata ricorsiva!
    if (node->left) {
...
int DecisionTree::fit(const Dataset& train_data) {
    ...
    // 2. Training Classico (con puntatori)
    ...
    // 3. Conversione Finale (Linearizzazione)
    // Pre-allochiamo un po' di memoria per evitare troppe riallocazioni
...
int DecisionTree::predict(const vector<double>& row) const { 
    if (flat_nodes.empty()) return 0; // Sicurezza
    return predict_one_flat(0, row);  // Parte dalla radice (indice 0)
}
