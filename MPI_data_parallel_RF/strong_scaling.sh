...
date >> risultati.txt
# varying number of nodes, strong scaling == fixed problem size == 256 trees
for n in 1 2 4 6 8; do
    ...
    srun --nodes=$n --ntasks-per-node=1 --cpus-per-task=32 --time=00:20:00 --mpi=pmix ./RandomForest_hybrid ../magic_dataset/magic04.csv 256 100 >> risultati.txt

    srun --nodes=$n --ntasks-per-node=1 --cpus-per-task=32 --time=00:20:00 --mpi=pmix ./RandomForest_hybrid ../covertype_dataset/covtype.csv 256 100 >> risultati.txt
