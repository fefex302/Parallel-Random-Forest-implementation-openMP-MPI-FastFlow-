...
for t in 1 2 4 8 16 32; do
    ...
    for n_trees in 1 50 100 250; do
        ...
        for i in 1 2; do
            ...
            srun --nodes=1 --ntasks-per-node=1 --time=00:10:00 ./RandomForest $t ../iris_dataset/iris.csv $n_trees 100 >> risultati.txt

            elif [ $i -eq 2 ]; then
                echo "Dataset: magic04.csv" >> risultati.txt
            srun --nodes=1 --ntasks-per-node=1 --time=00:10:00 ./RandomForest $t ../magic_dataset/magic04.csv $n_trees 100 >> risultati.txt
            # else
